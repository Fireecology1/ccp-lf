...
ISI.sa) and SFC. Notably, these models produced lower ROS predictions
than suggested by Van Wagner's [-@vanwagner1993] surface fire model, at
least for the range in question, representing ISI (or ISI.sa) values of
\~5-10 or so.
...
Using Byram's equation along with our empirical surface ROS models and
scaled ladder fuel equation [7] we can calculate the most likely $M$
value required to generate equivalent values of $I_0$ for crowning.

To do this we use treatments means for SFC and SFC_L along with

```{r}

#equivalent Byram's intensity near crowning threshold:
#only really works with mean SFC from both sites
sf.crisi.fun(6.61) * 300 * (1.34 + mean(sh.ctrl2$sfc.add) * 2.2)
sf.crisi.fun(10.31) * 300 * 1.34
