...
canopy), finding the surface-equivalent $FC$ value ($FC_{SE}$) is then a
matter of inflating its influence due to its elevated position. In this
case, z remains the normal LCBH for the live canopy and $z-z_L$ can also
be considered the fuel strata gap between ladder fuel elements and the
LCBH:
...
($z-z_L=1$) scales to equate to ground-level SFC of 1.12 kg m^-2^ , but
...
The final solution for estimating M was calculated by solving for M:
...
$M=4.6$ using the ISI.sa-based model and thresholds.[^1]

[^1]: Measured SFC values were not significantly different between
    treatments (p=0.07), so the mean value between all plots was used.
...
value for M was 3, a rough mean of the previous estimates.

As the table shows, the cross-validation evidence does support the
existence of M \> 1, but it is perhaps underwhelming. The best model
results (highest MCC, lowest AIC) for both FFMC and MC.SA-based models
are the versions using the M=3 multiplier for $FC_{SE}$.
