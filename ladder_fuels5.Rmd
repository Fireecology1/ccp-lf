...
sh.ctrl2 <- sh.ctrl %>% select(Plot, ISI, isi.m, FSG, SFC, sfc.add, CFI=CFI1) %>% 
...
sh.th2 <- sh.th %>% select(Plot, ISI, isi.m, FSG=FSG1, SFC, sfc.add, CFI=CFI1) %>% 
  mutate(sfc.both=SFC+sfc.add)
...
Crown fires, CEVW model and ladder fuels; short lit. review (Stephens'
papers, lab studies, etc.) Conifer forests cover most of the
circumboreal region of the world and crown fires represent a significant
fire-related hazard in these stands [@kneeshaw2011; @stocks1991]. As
noted by Kiil et al. [-@kiil1976] and quoted by Cruz et al.
[-@cruz2005], surface fires rarely spread \> 6 m/min under closed
conifer stands, as higher spread rates are usually associated with some
degree of crowning or torching. Once flames reach the upper reaches of
the canopy, they can be directly influenced by open air winds in the
lower atmosphere. The fully developed crowning process has long been
known to be associated with a dramatic increase in fire spread rate, due
to several processes including flame tilt, increased lee-side wind
exposure [@taylor2004] and vigorous ember spotting .
...
under the trees' [@wright1932]. Van Wagner [@vanwagner1977], in
...
position, by definition [@vanwagner1977; @kilgore1975]. They exist in
...
an advancing fireline), the heat flux contribution [@melnik2022] is much
closer to the canopy base, and therefore much more likely to contribute
to canopy ignition compared to similar fuel loading at the surface. The
structure of the Van Wagner [@vanwagner1977] CFI model permits a
vertical rescaling that relies on an implied equivalency between SFC and
the inverse of z (LCBH).

When we replace $I_0$ (Equation 1) with Byram's (1959) fireline
intensity, we get the following equation, a much-used equivalency (e.g.
FCFDG 1992, Scott and Reinhardt 2001):
...
As discussed, this relationship has often been evaluated in terms of
identifying critical ROS for crown fire initiation (Van Wagner 1977,
...
two different z values (equation 3). Using Van Wagner's own empirical
heat of ignition function [@vanwagner1968], at 90% foliar moisture
content (FMC) and LCBH=2 m, Equation 1 suggests $I_0$ of about 417 kW/m,
the intensity of a low to moderate intensity surface fire. At some
moderate surface ROS value, e.g. 3 m/min, Equation 3 predicts $SFC_0$ of
0.463 $kg \cdot m^{-2}$. Using Equation 5, increasing LCBH to 5 m
without altering other inputs would raise $SFC_0$ to 1.83
$kg\cdot m^{-2}$ ; thus, a 2.5 times increase in LCBH would result in a
fourfold increase in fuel consumption needed for crowning as a
consequence of the much higher surface fire intensity required.

In the case of the consumption of an elevated ladder fuel quantity
$FC_L$ positioned at height above ground $z_L$ (such as the centroid of
a standing dead sapling, or sapling cohort, beneath a live conifer
canopy), finding the equivalent surface-equivalent $FC$ value
($FC_{SE}$) is then a matter of inflating its influence due to its
elevated position. In this case, z remains the normal LCBH for the live
canopy and $z-z_L$ can also be considered the fuel strata gap between
ladder fuel elements and the LCBH:

$$
FC_{SE}=(\frac {z} {z-z_L})^{1.5} \cdot FC_L ~~, ~~~~~~ z_L > z
$$
...
aeration-limited combustion environment [@rothermel1972; @keane2015;
@schwilk2015]. The general FBP System approach to SFC, where all
consumed surface and ground fuels are assumed to contribute to surface
fire intensity [@vanwagner1977; @fcfdg1992], may overemphasize dense
duff fuels that burn in post-frontal combustion and ignores factors such
as residence time [@nelson1988; @wotton2012]. In contrast, fine crown
and ladder fuel strata are much more aerated (more porous) but also
vertically and horizontally discontinuous (refs and explanation#). This
...
($z-z_L$) diminishes, $FC_G$ can grow to extreme levels, eventually
producing a division by zero problem at $z=z_L$. Thus, 0.1
$kg \cdot m^{-2}$ of fuel consumption 1 m below a 5 m tall canopy base
scales to equate to ground-level SFC of 1.12 $kg \cdot m^{-2}$ , but the
same fuel consumption 0.2 m below the same canopy scales to
$FC_G=12.5~ kg \cdot m^{-2}$. This logic holds in a theoretical sense --
a fire is much more likely to ignite the lower live canopy if the
burning fuel is immediately beneath, and a 0.2 m separation is
essentially showing negligible separation at the fuel particle scale;
however, this suggests that caution is needed to utilize equation 6 in a
practical sense to avoid inflating the ladder fuel effect to illogical
levels.
...
Recall that SFC values in Canadian experimental fires ranged from 0.2 to
3.8 kg m\^{-2} [@perrakis2023]. The value of this multiplier, $M$, could
range from 1 (no effect) to 10 or more in the event that ladder fuel
biomass and arrangement appeared to have more than an order of magnitude
greater effect than surface biomass on upward heat flux and crown fire
initiation.
...
Sharpsand Creek, Ontario. Stand and fuel measurements were initiated in
the early 1970's at this site north of Thessalon, Ontario, in advance of
experimental burns conducted over a 15-year period. This resulted in 27
separate and independent experimental fire observations in three
distinct structure classes. The original unaltered stand (immature: IM)
consisted of densely stocked jack pine natural regenerated on gravelly
soil [@walker1975a] following the spring 1948 Mississagi fire
[@stocks1973]. The dense stands in the earliest burns featured over 9000
live stems/ha and over 10,000 dead stems/ha as the stand underwent rapid
...
several other fire behaviour studies [@call1997; @cruz2003;
@albini1986].
...
and site characteristics as the denser IM (C-4 fuel type) stands, the
burns in the thinned plots had been deemed more representative of mature
pine stands and previously included in the FBP System C-3 fuel type
dataset [@degroot2022].

Burning years ranged from in 1974-1976, 1981, 1988, 1990-1991; thus, at
post-fire ages 26-43. As discussed in a previous analysis
[@Perrakis2023], earlier studies handled these three treatments in
different ways, but mainly by estimating the ladder fuel influence as a
modification of the LCBH in a subjective sense, using an expert judgment
adjustment and defining the fuel strata gap (FSG) as the net distance
between surface and fuel strata, including the effects of standing dead
ladder fuels [@cruz2004, @cruz2006, @perrakis2023].
...
with experimental leaders (BJ Stocks, pers. comm.), the two fires
described as 'some torching' were split, with fire #16 (ISI=8.1, ROS=3.5
m/min) was considered a vigorous surface fire, while fire #18 (ISI=9.4,
ROS=5.1 m/min) was considered a marginal passive crown fire. As noted in
a previous study, as surface fires begin to involve more canopy fuels
and transition to passive crown fire, ROS is observed to increase beyond
the baseline for true surface fires (i.e., no crown fuel involvement).
...
(fine snags and dead crown fuel \< 1 cm; Stocks 1987) into an equivalent
SFC contribution value, $SFC_L$. The vertical position of fine standing
ladder fuels was assumed to be the crown centroid of the mean dead snag
height [LCBH+(height-LCBH)/2; @perrakis2023] in an experimental plot
(Figure X#snag diagram). SFC_L was then added to the actual (measured)
SFC for modelling. Additional tests were completed where the elevated
fuel contribution was increased using a multiplier, discussed further
below.
...
values as a first estimate along with equation 6 to investigate a
...
between treatments [in terms of ISI, the Initial Spread Index;
@vanwagner1987]. We then evaluated the crowning thresholds in terms of
critical FC to solve for the multiplier effect (M). This involved using
Byram's [-@byram1959] fireline intensity along with predicted ROS using
fitted surface ROS models, and solving for M using the threshold values.
...
content, $mc_{SA}$). This time, rather than presuming the FSG
adjustments to LCBH in certain stands, we used eq. 6 to estimate the
...
multiplier M in this case was held inside the log(SFC) term; thus, the
respective b_n was fitted to log(SFC+FC_SE x M). The initial value of M
in this fitted model was taken from the previous estimates, but a
sensitivity analysis step also tested other values of M, from 1 to 20.

## Results

#### Expert opinion

The first estimation method for the multiplier M was based on the expert
opinion assessment of Cruz et al. [@cruz2004; @cruz2005]. Starting with
the approximate LCBH values for the Sharpsand IM stands (4 m), this
value was reduced by 2 m to produce the FSG estimate. Therefore, $z=4$ m
and $z-z_L=2$ m, producing our first estimate for M:
...
contrasts were non-significant (p\>0.2 for all other pairs).
...
identify the most likely ISI crowning thresholds between treatments
(Figure XX). This produced the three logistic curves shown in Figure X.
By solving for p=0.5 (50% probability), we calculated the crown fire
transition thresholds based on ISI or stand-adjusted ISI (ISI.sa),
simply a renewed calculation of the ISI based on the FWI System
equations, using the stand-adjusted moisture content estimate [from
@wotton2007] in place of the FFMC-based moisture content estimate ['m'
in @vanwagner1987].
...
Crowning thresholds, as shown in the figure, were `r isi.thresh.im` and
`r isi.thresh.th` for immature controls and thinned stands,
respectively. In terms of ISI.sa, the $MC_{SA}$-based spread index, the
respective thresholds were `r isim.thresh.im` and `r isim.thresh.th` for
unthinned controls and thinned stands. Given identical ignition patterns
and stand origin, the difference between these values, therefore,
represents the increased crowning threshold achieved by reducing ladder
fuels, counteracted somewhat by reduced stand density (from 'dense' to
'moderate', based on Wotton and Beverly's [-@wotton2007] model).
...
best surface fire models were obtained using the combination of ISI or
ISI.sa and SFC, with adjusted R^2^ values of 0.747 and 0.807,
respectively. Notably, these models produce lower ROS predictions than
Van Wagner's surface fire model (VW93), 'to define the spread rate of
all possible surface fires' in the Sharpsand stands [@vanwagner1993], at
least for the range in question. While sROS predictions appear more
scattered above ISI 12 or so, conditions favoring crown fire in many
stands with lower LCBH, they are more clustered under lower danger
conditions. Based on the estimated crowning thresholds, representing ISI
(or ISI.sa) values of \~5-10, surface fire spread rates of 0.7 – 2.5 m
min^-1^ are expected, notably less than the 2 – 10 m min^-1^ values
predicted by the VW93 model.
...
equivalent $I_0$ are required for crowning in the control and thinned
stands, both with LCBH of 4.4 m.
...
($z_L$) using an estimated crown ratio of 0.5 and cohort heights from
...
m and 0.159 kg m^-2^, resulting in mean $z_L=2.62$ m. Scaled
surface-equivalent consumption, calculated using [6], ranged from 0.291
to 1.227 kg m^-2^, with a mean of 0.658 kg m^-2^ .

The final solution for estimating M was calculated as follows:

$$\begin{align*}
SFC_{Th} \cdot ROS_{0 \cdot Th}
incomplete
\end{align*}
$$

Using overall mean SFC values (1.34 kg m^-2^), this generates estimates
of $M = 2.2$ using the ISI-based sROS model and crowning thresholds, and
$M=4.6$ using the ISI.sa-based model and thresholds.

Note that the actual measured SFC values were somewhat different between
treatments, with thinned stands (marginally significant: Welch's
t=0.0773). If we use actual treatment mean SFC values of
...
#Using isi.m models:
sf.crisim.fun(4.7) * 300 * (1.34 + mean(sh.ctrl2$sfc.add) * 4.6)
sf.crisim.fun(9.19) * 300 * 1.34 
