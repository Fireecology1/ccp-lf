...
knitr::opts_chunk$set(echo = FALSE, root.dir = "c:/Dan/_Remote_projects/ccp_git/ccp-cfi")
...
fd4 <- read.csv('c:/Dan/_Remote_projects/ccp-lf/files/fire_data_july2023.csv') %>%  
  mutate(CFI2=factor(CFI1, labels=c('Surface', 'Crown')))

setwd('c:/Dan/_Remote_projects/ccp-lf/files')
...
fires <- filter(fd4, num %in% c(sharp, sharp.th, sharp.sm)) %>% 
  ...
  mutate(Exp=case_when(
         ...
         Plot=str_extract(fire, '\\d+') %>% as.integer) 
fires <-  mutate(rowwise(fires), MC.SA2=wbmc(ffmc=FFMC, dmc=DMC, stand=4, density=2, season=MC.SA_season)) 
...
fires <- mutate(rowwise(fires), 
                isi.m=isi.mcsa(mc=MC.SA, ws=ws),
                isi.m2=isi.mcsa(mc=MC.SA2, ws=ws))
```

# A Simple solution for ladder fuels {#Title, Abstract}
...
Crown fire represents the most dangerous type of potential fire
behaviour in conifer forests and predicting their onset remains
uncertain. Many operational modelling systems rely on the C.E. Van
Wagner (Can. J. For Res. 1977, v7: 23-34) crown fire initiation model, a
one-dimensional equation developed from simple physical theory and
laboratory experiments. Van Wagner's model requires a clear vertical
separation (\> \~ 1m) between surface fuels and a relatively defined
canopy base; any fine-textured biomass between the surface and canopy
fuels are considered ladder fuels, generally not accounted for by the
model but believed to be disproportionately influential in crown fire
initiation. To date, techniques for estimating the contribution of
ladder fuels are lacking. In this study, we show how a simple
rearranging of Van Wagner's model, by solving for critical surface fuel
consumption (SFC), allows for the comparison of fuel consumption at
different vertical positions in the mid-story. This effectively allows
ladder fuel consumption to be treated as a scaled equivalent of SFC,
although a calibration step is required due to significant differences
in fuel bulk density between dense surface fuelbeds and well-aerated
ladder fuels. We used fire observations and crown fire thresholds from a
well- documented series of experimental burns in immature jack pine
stands to estimate a multiplier for ladder fuel loads; a second estimate
was tested based on previously published expert opinion. These two
analyses produced a first estimate for a multiplier of 2-3x##, meaning
that ladder fuel consumption is about two to three times as influential
per unit mass as compact surface fuel consumption. This method provides
users with a simple option to evaluate ladder fuel influence in crown
fire models or hazard reduction treatments.
...
[@kneeshaw2011, @stocks1991]. As noted by ##Cruz ref. surface fires
...
several processes including flame tilt and vigorous ember spotting.

(#keep?) A recent analysis of Canadian experimental burns in boreal and
...
The element in fuel complexes that enables crown fires in most tall
...
height'. While the paper describing Van Wagner's model has become a
classic with thousands of citations, methods for quantifying the effects
...
The theoretical relationship can be examined by comparing $SFC_0$ at two
different z values. For example, using Van Wagner's own empirical heat
...
$FC_L$ positioned at height $z_L$, in this case the vertical centroid of
...
Using Equation 6, it should be possible to scale the contribution of
...
than crown fuel complexes, with a much more aeration-limited combustion
...
The objective of this study was to present a theoretical framework for
analyzing ladder fuel influence in conifer crown fire initiation using a
modification of Van Wagner's (1977) equation. The presented theory was
then evaluated using the expanded set of experimental fires from
Sharpsand Creek, Ontario, Canada, a well-known boreal jack pine study
site, as well as by incorporating ladder fuel influence into a
recently-published empirical crown fire initiation model. By comparing
estimated and observed fire behaviour stands with varying ladder fuel
densities, we assessed the effectiveness of the theoretical model for
improving overall fire behaviour predictions.

###Get data

``````{r get.data}
#pick which Sharp-SM fires are crown vs surface
some.torch <- c(99)   #change 'some torching' fires to surface: 99, 101 (maybe)

fires2 <- fires   #New CFI feature for shape mapping
fires2$CFI1[which(fires2$num %in% some.torch)] <- 0   #comment if undesired
fires2$CFI2[which(fires2$num %in% some.torch)] <- 'Surface'   #comment if undesired
#fires2 <- mutate(fires2, CFI2=as.factor(CFI))

#fires3 <- fires2 %>% filter(Exp %in% c('Thin', 'Ctrl'))  #Not needed for now
```
...
The findings from the experimental fires at Sharpsand Creek have
previously been analyzed on several occasions. We present old results
here only in the context of new analysis.

(probably omit histogram)

### #Sharpsand histograms with 3 treatments

```{r Sharpsand Histograms}
scalex <- seq(2, 22, by=2)

#graph with histograms: ISI
sh.lad.histo <- ggplot(fires2, aes(x=ISI)) + 
...
sh.lad.histo2 <- ggplot(fires2, aes(x=isi.m)) + 
  geom_histogram(binwidth=1, aes(fill=CFI2)) + 
  facet_grid(Exp~.) +
  scale_x_continuous(breaks=scalex) +
  theme_classic() +
...
### #Pooling SM and TH, new histogram
...
fires.stat <- select(fires2, CFI1, CFI2, ISI, Exp) 
#%>%  mutate(CFI2=as.numeric(CFI2))

#sh.mod <- glm(CFI ~ ISI + Exp, data=fires2, family='binomial')
sh.mod2 <- glm(CFI2 ~ isi.m + Exp, data=fires2, family='binomial')
sh.mod22 <- glm(CFI2 ~ isi.m2 + Exp, data=fires2, family='binomial')
sh.modB <- glm(CFI2 ~ ISI + Exp, data=fires2, family='binomial') 
sh.mod.ff <- glm(CFI2 ~ ws + MC.SA2 + Exp, data=fires2, family='binomial') 

#graph with jittered scatter plot
#Isi.seq <- seq(4, 20, by=0.1)

#which model to use? sh.mod2 or sh.modB or other
use.mod <- sh.modB
...
sh.isi.log <- 
  ggplot(fires.stat, aes(y=CFI1, x=ISI))+
...
Immature: `r isi.thresh.imm()` #6.53

Thinned: `r isi.thresh.th()` #10.29

Semi-mature: `r isi.thresh.semiM()` #8.61

At median WS_10 of 12.3 km /h, the IM threshold works out to FFMC of
about 88.6; TH threshold is about FFMC of 91.8, and SM threshold is
about FFMC of about 90.5. We will use both Van Wagner's (1977) and
Perrakis et al's (2023) crown fire models to calibrate the SFC
multiplier for the ladder fuel consumption using these threshold values.

Immature:

In the SH-IM stand, mean LCBH was 4.36 m, and mean SFC was 1.444
kg/m\^2. The crowning threshold of ISI `r isi.thresh.imm()` #6.53 is
equivalent to FFMC of 88.61 at mean overall ws of 12.3 km/h. With a dead
stem mean height of 3.49 m, the ladder fuel centroid $C_L=$ 1.75 m, and
$z-z_L=$ 2.62 m. Ladder fuel consumption, based on the loading of \< 1
cm dead roundwood (Stocks 1987), is estimated at 0.159 kg/m2. Using
equation 6, this scales to $FC_G=$ 0.3392 and an effective SFC (SFC +
scaled-to-ground ladder fuel consumption) is SFC_e=1.444 + 0.3392 M.
Using equation 1, at mean FMC=102% and z=4.36m, $I_0$=1573 kW m-1.

A rearrangement of Byram's (1959) equation using the effective SFC
allows us to solve for M. This presentation uses ROS_0, the critical ROS
for crowning in a given stand [@vanwagner1993]. We could solve for M
using the mean SFC and I_0 attributes for both the TH and IM stands, but
will need to estimate at least one ROS_0 value in order to solve the
equation.

$$SFC_e=\frac {I_0}{h \cdot ROS_{0}}$$
