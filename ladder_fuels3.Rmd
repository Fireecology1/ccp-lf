...
d1 <- read.csv('c:/Dan/_Remote_projects/ccp-lf/files/fire_data_d1.csv')
...
sh.fires1 <- mutate(rowwise(sh.fires0), MC.SA2=wbmc5(ffmc=FFMC, dmc=DMC, stand=4,
...
The objectives of this study were 1- to present a theoretical framework for
analyzing ladder fuel influence in crown fire initiation using a simple
reworking of Van Wagner's (1977) equation; 2- to fit simple surface rate of spread models based on Canadian experimental data; and 3- to evaluate how incorporating ladder fuel contributions to crowning compares using both theoretical and empirical models using experimental crown fire data. The presented theory was then evaluated using the expanded set of experimental fires from Sharpsand Creek, Ontario, Canada, a well-known boreal jack pine study site, as well as by incorporating ladder fuel influence into a recently-published empirical crown fire initiation model. By comparing estimated and observed fire behaviour stands with varying ladder fuel
densities, we assessed the effectiveness of the theoretical model for improving overall fire behaviour predictions.
...
#New CFI feature for shape mapping
sh.fires$CFI1[which(sh.fires$num %in% some.torch)] <- 0   #comment if undesired
sh.fires$CFI2[which(sh.fires$num %in% some.torch)] <- 'Surface'   #comment if undesired
...
fires.stat <- select(sh.fires, CFI1, CFI2, ISI, Exp, isi.m, isi.m2) %>%
  ungroup()
...
sh.mod2 <- glm(CFI1 ~ isi.m2 + Exp, data=sh.fires, family='binomial') #ISI using MC.SA, density 2 for TH, 3 for others; marg. sig. for trt (p=0.0606)
...
###Objective
 - Present new model, based on CEVW's CFI model, allowing for calculation of LF contribution to crown fire initiation and other variable height fuel consumption equivalency
 - Evaluate model using data from several existing experimental datasets
 
##Methods
###Rescaling model: bringing LF to the surface
