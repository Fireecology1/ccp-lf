...
s.fires.con <- filter(s.fires, FT!='Decid') 
...
#r^2=0.7182 with s.fires2
#r^2=0.679 with s.fires.mod

#lm using isi.m
sros.isim2.mod <- lm(ROS ~ I(isi.m^2)-1, data=s.fires.mod)
#r^2=0.687 with s.fires2
#r^2=0.749 with s.fires.mod

#lm using isi.m AND SFC
sros.isim2SFC.mod <- lm(ROS ~ I(isi.m^2)+SFC-1, data=s.fires.mod)
#r^2=0.735 with s.fires2
#r^2=0.796 with s.fires.mod
...
sh.mod2 <- glm(CFI1 ~ isi.m + Exp, data=fires.stat, family='binomial') #ISI using MC.SA, density 2 for TH, 3 for others; marg. sig. for trt (p=0.0606)
...
fdata.IM <- data.frame(fISI=seq(3, 20, by=0.1), fExp='Imm') %>%
  mutate(CFI=predict.glm(use.mod, newdata=list(isi.m=fISI, Exp=fExp), 
                         type='response'))
...
We will work with the Immature and Thinned thresholds to solve for M, using the ISI and ISI.mcsa-based crown fire occurrence and surface fire spread models. 
...
Using Byram's [-@Byram1959] equation, we will use the crown fire threshold values along with the SROS models to to calibrate the SFC multiplier for ladder fuel consumption:

$$I_0=h \cdot SFC \cdot ROS_0$$
We will use the FBP System conventions for units and constants: I_0 in kW m-1, SFC in kg m-2, ROS in m min-1, and h=18,000 kJ kg-1; to convert ROS to m s-1, we also multiply by the constant 1 min/60 s  [@FCFDG 1992]. The final assumption is that the LCBH of both the Immature and Thinned stands are identical at 4.36 m. Stocks [-@stocks1987], Van Wagner [-@vanwagner1993] and the FBP System [@FCFDG 1992] all characterized  
...
kg/m\^2. The crowning threshold is ISI `r isi.thresh.imm()` #6.61 or ISI.m #4.70. 
With a dead stem mean height of 3.49 m, the ladder fuel centroid $C_L=$ 1.75 m, and
...
equation 6, this scales to $FC_G=$ 0.3392 kg/m2. We now define the effective SFC as  $SFC_e=SFC + FC_G \cdot M$ (actual SFC plus scaled-to-ground ladder fuel consumption times a multiplier). Thus, $SFC_e=1.444 + 0.3392 M$. Using equation 1, at mean FMC=102% and z=4.36m, $I_0=1573kW m^{-1}$.
